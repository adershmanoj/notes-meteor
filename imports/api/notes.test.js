...
if(Meteor.isServer){
  describe('notes', function(){
    ...
    it('should not allow extra updates', function(){
      const visible = true;
      expect(()=>{
        Meteor.server.method_handlers['notes.update'].apply({userId: noteOne.userId}, [noteOne._id, {visible}]);
      }).toThrow();
    });
    ...
    it('should not update note if unauthenticated', function(){
      expect(()=>{
        Meteor.server.method_handlers['notes.update'].apply({}, [noteOne._id, noteOne]);
      }).toThrow();
    });
    ...
    it('should return 0 for user with no notes', function(){
      const res = Meteor.server.publish_handlers.notes.apply({userId: 'test-3'});
      const notes = res.fetch();
